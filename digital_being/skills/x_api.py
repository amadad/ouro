...
from framework.composio_connection import ensure_connection
...
class XAPISkill:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.posts_count = 0
        self.twitter_username = config.get("twitter_username", "YourUserName")  # Get from character config
    ...
    async def upload_media(self, media_url: str) -> Optional[str]:
        ...
        """
        # Make sure we're connected to TWITTER_MEDIA
        if not ensure_connection("TWITTER_MEDIA", interactive=False):
            logger.error("Not connected to TWITTER_MEDIA. Cannot upload media.")
            return None
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(media_url) as response:
                    ...
                    upload_response = composio_manager.execute_action(
                    ...
                    # Check for success in both places it might be
                    if upload_response.get("successful") or upload_response.get("success"):
                        # Handle different response formats
                        media_id = None
                        
                        # Try to get media_id from data.media_id
                        if "data" in upload_response:
                            media_id = upload_response.get("data", {}).get("media_id")
                        
                        # If not found, look in other possible locations
                        if not media_id and "media_id" in upload_response:
    ...
    async def post_tweet(self, text: str, media_urls: List[str] = None) -> Dict[str, Any]:
        ...
        # Check that we're connected to Twitter first
        if not ensure_connection("TWITTER", interactive=True):
        ...
        try:
            ...
            media_ids = []
            if media_urls:
                # Check for TWITTER_MEDIA connection if we have media
                if not ensure_connection("TWITTER_MEDIA", interactive=True):
                    logger.warning("Not connected to TWITTER_MEDIA. Media uploads will fail.")
                else:
            ...
            # Prepare parameters based on the Composio API for Twitter
            params = {"text": text}
            if media_ids:
                # Use the correct parameter format for media IDs
                # This might be different depending on Composio's API
                params["media_ids"] = media_ids  # Standard format
                # Also try alternate formats that might be needed
                params["media__media__ids"] = media_ids

            response = composio_manager.execute_action(
